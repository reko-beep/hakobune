...
from data_client.utils import URL
from bs4 import BeautifulSoup, Tag, NavigableString
from os.path import exists
from os import makedirs
from json import dump, load
from utils import logc
class BaseScraper:
    '''
    ...
    def __init__(self, client, url: URL) -> None:
        ...
        self.type = 'misc'
        from data_client.main import DataClient
    ...
    def fetch(self, force: bool = False):
        ...
        if force:
            ...
            if self.client.pending_update(self.key):
                
                logc(f'Fetched data for {str(self)}')
                self._scrape()
                self.save_to_file()
            else:
                self.data = self.client.get_data(key)
    ...
    def __check_path(self):
        '''
        private function that checks if type folder exists
        and if not makes it
        '''

        path = f'{self.base_path}/{self.type}/{self.filename}'

        if not exists(f'{self.base_path}/{self.type}'):
            mkdir(f'{self.base_path}/{self.type}')


    def save_to_file(self):
        '''
        saves the fetched data to file
        '''
        self.__check_path()
        
        with open(f'{self.base_path}/{self.type}/{self.filename}', 'w') as f:
